...
?>
<?php function consultaInformacionPlan($empresa, $servicio="internet"){
    ...
    foreach( $planes as  $plan):
        echo '<div class="plan">';
            ...
            echo '<div class="detalle">';
                echo '<ul>';
                    echo '<li>Precio: $'.'<span>'.$plan["precio"].'</span>'.'</li>';
                    foreach($plan[0] as $detalle):
                        echo '<li>'.$detalle["descripcion"].' <span>'.$detalle["caracteristica"].'</span>'.'</li>';
                    endforeach;
                echo "</ul>";
        ...
        echo '<div class="detalle plan-seleccion" style="display: block;">';
            echo '<ul>';
                ...
                foreach($plan[0] as $detalle):
                    echo '<li>'.$detalle["descripcion"].' <span>'.$detalle["caracteristica"].'</span>'.'</li>';
                endforeach;
                echo '<br><br><li>Precio: $'.'<span>'.$plan["precio"].'</span>'.'</li>';
            echo "</ul>";
        echo "</div>";
    echo '</div>';
    return $plan;
